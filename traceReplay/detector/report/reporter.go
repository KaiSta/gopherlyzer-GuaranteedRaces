...
//divide by type should not report doubles only because they are a different race type. makes the evaluation easier!!!!
func ReportRace(loc1, loc2 Location, isWrRdDep bool, level int) bool {
	if divideByType {
		return RaceStatistics2(loc1, loc2, isWrRdDep, level)
	}

	//QUICKFIX to avoid double entries
	b1 := loc1.W
	b2 := loc2.W
	loc1.W = true
	loc2.W = true
	...
	if !details {
		color.HiRed("RACE:%v,%v\n", loc1, loc2)
	} else {
		f1 := parser.FileNumToString[loc1.File]
		f2 := parser.FileNumToString[loc2.File]
		color.HiRed("RACE:%v:%v, %v:%v\n", f1, loc1.Line, f2, loc2.Line)

		if csv {
			if csvFile == nil {
				csvFile, _ = os.Create("result.log")
			}
	...
	fmt.Println(b1, b2)
	fmt.Printf("ALL:%v/%v\nWRDRaces:%v\n", writeWriteRaceUnique, writeWriteRace, writeReadDepRaceUnique)
	fmt.Println("WWRACES:", wwRace, "WRRACES:", wrRace, "RWRACES:", rwRace)
	return true
}

func ReportRaceDRY(loc1, loc2 Location, isWrRdDep bool, level int) bool {
	//QUICKFIX to avoid double entries
	loc1.W = true
	loc2.W = true
	//
