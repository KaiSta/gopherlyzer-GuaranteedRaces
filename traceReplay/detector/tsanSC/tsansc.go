...
func (l *ListenerAsyncRcvP3) Put(p *util.SyncPair) {
	...
	//remove now closed lock from all threads
	for _, t := range threads {
		if t.id != p.T1 {
			// cshist := t.csHistory[p.T2]
			// cshist = append(cshist, nPair)
			t.csHistory[p.T2] = make([]vcPair, 0) //only one thread can own it, so per lock there can always only be a single active critical section. []vcpair should always contain only a single event.
		}
	}
	...
	// //cleanup
	// for k := range t1.csHistory {
	// 	lkhist, ok := t1.csHistory[k]
	// 	if ok {
	// 		nhist := make([]vcPair, 0, len(lkhist))
	// 		for _, p := range lkhist {
	// 			if p.rel.get(p.owner) > t1.vc.get(p.owner) {
...
func (l *ListenerAsyncSndP3) Put(p *util.SyncPair) {
	...
	if len(cshist) > lock.count && len(cshist) > 0 {
		nPair := cshist[lock.count]
		//append open lock to all threads
		for _, t := range threads {
			if t.id != p.T1 {
				cshist := t.csHistory[p.T2]
				cshist = append(cshist, nPair)
...
func (l *ListenerDataAccessP3) Put(p *util.SyncPair) {
	...
	newFE := &dot{v: t1.vc.clone(), int: varstate.current, t: uint16(p.T1),
		...
		write: p.Write, ls: make(map[uint32]struct{}), pos: p.Ev.LocalIdx, csHistory: make(map[uint32][]vcPair)}
	for _, k := range t1.ls { //copy lockset of thread
		newFE.ls[k] = struct{}{}
	}
	for k, v := range t1.csHistory {
		if len(v) > 0 {
			newFE.csHistory[k] = append(v[:0:0], v...) //copy v to history (:0:0 is a trick to make it more efficient)
		}
	}
				...
				if !intersect(newFE.ls, f.ls) {
					...
					if !f.promoted {
						for k := range locks {
							...
							hist, ok := f.csHistory[k]
							if !ok || len(hist) == 0 {
								continue
							}

							for _, p := range hist {
								localTimeForLastOwner := f.v.get(p.owner)
								if p.rel.get(p.owner) < localTimeForLastOwner {
									continue
								} else {
									relTimeForLastOwner := p.rel.get(uint32(f.t))

									if p.acq.v < localTimeForLastOwner && f.v.get(uint32(f.t)) < relTimeForLastOwner {
										f.ls[k] = struct{}{}
									}
								}
							}
		...
		if varstate.lwDot != nil {
			...
			if k > thi_at_j {
				if !intersect(newFE.ls, varstate.lwDot.ls) {
					...
					if !varstate.lwDot.promoted {
						for k := range locks {
							if _, ok := varstate.lwDot.ls[k]; ok {
								continue
							}
							hist, ok := varstate.lwDot.csHistory[k]
							if !ok || len(hist) == 0 {
								continue
							}

							for _, p := range hist {
								localTimeForLastOwner := varstate.lwDot.v.get(p.owner)
								if p.rel.get(p.owner) < localTimeForLastOwner {
									continue
								} else {
									relTimeForLastOwner := p.rel.get(uint32(varstate.lwDot.t))

									if p.acq.v < localTimeForLastOwner && varstate.lwDot.v.get(uint32(varstate.lwDot.t)) < relTimeForLastOwner {
										varstate.lwDot.ls[k] = struct{}{}
									}
								}
							}
						}
						varstate.lwDot.promoted = true
					}
...
func Init() {
	...
	algos.RegisterDetector("tsanSC", &EventCollector{listenersp1, listenersp2, listenersp3, 1})
}
