...
func (v epoch) union(v2 vcepoch) vcepoch {
	v3 := newvc2().internal_set(v.t, v.v)
	return v3.ssync(v2)
}
...
func (v vc2) union(v2 vcepoch) vcepoch {
	switch x := v2.(type) {
	case vc2:
		return newvc2().ssync(x).ssync(v)
	default:
		panic("union with unsupported type!")
	}
	return newvc2()
}
