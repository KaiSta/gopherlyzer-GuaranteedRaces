...
func WriteTrace(trace []util.Item, logfilePath string) {
	...
	for _, it := range trace {
		eventType := "WR"
		switch it.Ops[0].Kind {
		case util.READ:
			eventType = "RD"
		case util.WRITE:
			eventType = "WR"
		case util.ATOMICREAD:
			eventType = "ARD"
		case util.ATOMICWRITE:
			eventType = "AWR"
		case util.SEND | util.POST:
			eventType = "LK"
		case util.RCV | util.POST:
			eventType = "UK"
		case util.SIG:
			eventType = "SIG"
		case util.WAIT:
		...
		filename := FileNumToString[it.Ops[0].SourceRef]

		eventLine := fmt.Sprintf("%v,%v,%v,%v:%v:%v,%v\n", it.Thread, eventType,
			it.Ops[0].Ch, filename, it.Ops[0].Line, 0, counter)

		_, err = io.WriteString(logfile, eventLine)
		if err != nil {
			panic(err)
		}
		counter++
