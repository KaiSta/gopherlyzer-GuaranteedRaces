...
func TestThreeWrites(t *testing.T) {
	...
	if raceCount != 3 {
		t.Errorf("Expected three race, got %v!", raceCount)
	}
	fmt.Println("-----------------------------------------------")
}
...
func TestWrdOp(t *testing.T) {
	...
	wr2 := util.SyncPair{T1: 1, T2: 2, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 1, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 2}}}}
	wr3 := util.SyncPair{T1: 2, T2: 1, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 2, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 4}}}}
	wr4 := util.SyncPair{T1: 3, T2: 1, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 3, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 6}}}}
	rd1 := util.SyncPair{T1: 2, T2: 2, DataAccess: true, Read: true,
		Ev: &util.Item{Thread: 2, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 3}}}}
	rd2 := util.SyncPair{T1: 3, T2: 2, DataAccess: true, Read: true,
		Ev: &util.Item{Thread: 3, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 5}}}}
	...
	report.TestFunc = func(l1, l2 report.Location, imp report.Level) {
		raceCount++
		if !(l1.Line == 2 && l2.Line == 3) && !(l1.Line == 2 && l2.Line == 5) && !(l1.Line == 4 && l2.Line == 6) {
			t.Errorf("2Wrong Race %v %v %v", l1, l2, imp)
		}
	}
	...
	if raceCount != 3 {
		t.Errorf("Expected 3 races, got %v!", raceCount)
	}
	fmt.Println("-----------------------------------------------")
}
...
func TestRdRdRace(t *testing.T) {
	...
	rd1 := util.SyncPair{T1: 1, T2: 1, DataAccess: true, Read: true,
		Ev: &util.Item{Thread: 1, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 3}}}}
...
func TestX(t *testing.T) {
	...
	if raceCount != 4 {
		t.Errorf("Expected four races, got %v!", raceCount)
	}
	fmt.Println("-----------------------------------------------")
}
...
func TestFirstIsRead(t *testing.T) {
	...
	report.TestFunc = func(l1, l2 report.Location, imp report.Level) {
		raceCount++
		if !(l1.Line == 3 && l2.Line == 1) && !(l1.Line == 1 && l2.Line == 3) {
			t.Errorf("Wrong Race %v %v %v", l1, l2, imp)
		}
	}
...
func TestBugTransitiveHull(t *testing.T) {
	fmt.Println("TESTY")
	wr1 := util.SyncPair{T1: 1, T2: 1, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 1, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 1}}}}
	wr2 := util.SyncPair{T1: 2, T2: 1, DataAccess: true, Write: true,
	...
	wr4 := util.SyncPair{T1: 2, T2: 1, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 2, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 4}}}}
	rd1 := util.SyncPair{T1: 2, T2: 1, DataAccess: true, Read: true,
		Ev: &util.Item{Thread: 2, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 5}}}}
	wr5 := util.SyncPair{T1: 1, T2: 1, DataAccess: true, Write: true,
		Ev: &util.Item{Thread: 1, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 6}}}}
	rd2 := util.SyncPair{T1: 1, T2: 1, DataAccess: true, Read: true,
		Ev: &util.Item{Thread: 1, Ops: []util.Operation{util.Operation{SourceRef: 1, Line: 7}}}}

	syncpairs := []util.SyncPair{wr1, wr2, wr3, wr4, rd1, wr5, rd2}
	...
	if raceCount != 11 {
		t.Errorf("Expected 11 races, got %v!", raceCount)
	}
	fmt.Println("-----------------------------------------------")
}
